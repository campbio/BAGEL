...
cosmic_v3_sbs_sigs_exome <- new("Result", signatures = as.matrix(exome_mat), type = "Cosmic_v3_SNV_Exome")
usethis::use_data(cosmic_v3_sbs_sigs_exome, internal = TRUE)

v3_snv_tab <- read.table("./data-raw/sigProfiler_SBS_signatures_2019_05_22.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
snv_mat=v3_snv_tab[,3:ncol(v3_snv_tab)]
rownames(snv_mat)=paste(v3_snv_tab$Type, v3_snv_tab$SubType, sep="_")
cosmic_v3_sbs_sigs <- new("Result", signatures = as.matrix(snv_mat), type = "Cosmic_v3_SNV_Genome")
usethis::use_data(cosmic_v3_sbs_sigs, internal = TRUE)

v3_dbs_tab <- read.table("./data-raw/sigProfiler_DBS_signatures.csv", header = TRUE, sep = ",", row.names = 1, stringsAsFactors = FALSE)
row.names(v3_dbs_tab) = gsub('>', '>NN_', row.names(v3_dbs_tab))
cosmic_v3_dbs_sigs <- new("Result", signatures = as.matrix(v3_dbs_tab), type = "Cosmic_v3_DBS_Genome")
usethis::use_data(cosmic_v3_dbs_sigs, internal = TRUE)
